...
import { Users, Activity, Gauge, Clock, Coffee, Zap, Play, Pause, Square, Target, ShieldAlert, ArrowRight } from "lucide-react"
...
export function TeamLeadDashboard() {
  ...
  const pollDashboardData = async () => {
    try {
      const data = await getDashboardData(currentUser.id, currentUser.role || "team_lead")
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {me && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Real-time application and website usage breakdown for your active session today.</CardDescription>
          </CardHeader>
      ...
      {/* Team Lead's own activity timeline */}
      <Card>
        <CardHeader>
        ...
        <CardContent className="space-y-1">
          ...
          {recentActivity.filter(e => e.internId === currentUser.id).length === 0 && (
            <p className="text-sm text-muted-foreground italic py-4 text-center">No recent activity recorded for your account.</p>
          )}
        </CardContent>
      </Card>
    </div>
